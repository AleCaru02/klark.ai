-- Applied to the Lovable/Supabase production database on 2026-08-05.
-- Phase 1: browser token isolation and private voice audio.
...
BEGIN
  ...
  SELECT jsonb_build_object('connected',true,'calendar_id',g.calendar_id,'scope',g.scope,
    'token_expires_at',g.token_expires_at,'expired',(g.token_expires_at<=now()),'updated_at',g.updated_at)
  INTO v_google FROM public.google_tokens g WHERE g.tenant_id=v_tenant;

  SELECT jsonb_build_object('connected',true,'page_id',CASE WHEN f.page_id='__pending__' THEN NULL ELSE f.page_id END,
    'pending_page_selection',(f.page_id='__pending__'),'token_expires_at',f.token_expires_at,
    'expired',(f.token_expires_at IS NOT NULL AND f.token_expires_at<=now()),'updated_at',f.updated_at)
  INTO v_facebook FROM public.facebook_integrations f WHERE f.tenant_id=v_tenant;

  SELECT jsonb_build_object('connected',true,'waba_id',w.waba_id,'phone_number_id',w.phone_number_id,
    'display_phone_number',w.display_phone_number,'verified_name',w.verified_name,
    'token_expires_at',w.token_expires_at,'expired',(w.token_expires_at IS NOT NULL AND w.token_expires_at<=now()),
    'created_at',w.created_at,'updated_at',w.updated_at)
  INTO v_whatsapp FROM public.whatsapp_integrations w WHERE w.tenant_id=v_tenant;
...
GRANT EXECUTE ON FUNCTION public.get_integration_status() TO authenticated, service_role;
...
REVOKE ALL ON public.google_tokens, public.facebook_integrations, public.whatsapp_integrations FROM anon, authenticated;
GRANT ALL ON public.google_tokens, public.facebook_integrations, public.whatsapp_integrations TO service_role;
