...
describe("route pubbliche in modalità preview", () => {
  it("carica la landing senza richiedere il backend", async () => {
    window.history.pushState({}, "", "/");
    render(<App />);
    expect(await screen.findByRole("heading", { name: /una receptionist ai che risponde alle chiamate/i })).toBeInTheDocument();
  ...
  it("carica la richiesta demo e resta fail-closed senza backend", async () => {
    ...
    render(<App />);
    expect(await screen.findByRole("heading", { name: /raccontaci come gestisci le chiamate/i })).toBeInTheDocument();
  ...
  it("carica demo e carta del servizio senza dati esterni", async () => {
    ...
    render(<App />);
    expect(await screen.findByRole("heading", { name: /guarda cosa succede dopo la risposta/i })).toBeInTheDocument();
    cleanup();

    window.history.pushState({}, "", "/carta-servizio");
    render(<App />);
    expect(await screen.findByRole("heading", { name: /carta del servizio/i })).toBeInTheDocument();
  ...
  it("carica la pagina sul funzionamento senza mettere i provider al centro", async () => {
    ...
    render(<App />);
    expect(await screen.findByRole("heading", { name: /una receptionist affidabile non deve soltanto parlare/i })).toBeInTheDocument();
  ...
  it("carica pagine settore con contenuto e FAQ specifiche", async () => {
    ...
    render(<App />);
    expect(await screen.findByRole("heading", { name: /receptionist ai per ristoranti/i })).toBeInTheDocument();
  ...
  it("carica una 404 professionale", async () => {
    ...
    render(<App />);
    expect(await screen.findByRole("heading", { name: /questa pagina non esiste o è stata spostata/i })).toBeInTheDocument();
  ...
  it("carica il login unificato e segnala il backend non configurato", async () => {
    ...
    render(<App />);
    expect(await screen.findByRole("heading", { name: /accedi al tuo account/i })).toBeInTheDocument();
