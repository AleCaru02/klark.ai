(() => {
  ...
  style.textContent = `
    ...
    .launcher{position:fixed;right:20px;bottom:20px;z-index:2147483000;width:58px;height:58px;border:0;border-radius:999px;background:#111827;color:#fff;box-shadow:0 14px 38px rgba(15,23,42,.3);cursor:pointer;display:grid;place-items:center}
    ...
    .panel{position:fixed;right:16px;bottom:90px;z-index:2147482999;width:min(410px,calc(100vw - 24px));height:min(660px,calc(100dvh - 112px));display:none;flex-direction:column;overflow:hidden;background:#fff;color:#111827;border:1px solid #e5e7eb;border-radius:20px;box-shadow:0 24px 80px rgba(15,23,42,.28);font:14px/1.48 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.panel.open{display:flex}
    .header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;padding:15px 16px 13px;background:linear-gradient(135deg,#111827,#1f2937);color:#fff;flex:0 0 auto}.eyebrow{display:flex;align-items:center;gap:6px;font-size:10px;letter-spacing:.08em;text-transform:uppercase;opacity:.82;margin-bottom:2px}.status{width:7px;height:7px;border-radius:50%;background:#34d399;box-shadow:0 0 0 3px rgba(52,211,153,.15)}.title{font-size:16px;font-weight:760;line-height:1.25}.subtitle{font-size:12px;line-height:1.35;opacity:.82;margin-top:4px;max-width:300px}.close{border:0;background:transparent;color:#fff;cursor:pointer;font-size:24px;line-height:1;padding:0 2px;flex:0 0 auto}
    .messages{flex:1 1 auto;min-height:0;overflow:auto;background:#f8fafc;padding:12px;display:flex;flex-direction:column;gap:9px;scroll-behavior:smooth;-webkit-overflow-scrolling:touch}.msg{max-width:88%;border-radius:14px;padding:9px 11px;white-space:pre-wrap;overflow-wrap:anywhere;font-size:14px;line-height:1.48}.assistant{align-self:flex-start;background:#fff;border:1px solid #e5e7eb;color:#111827}.user{align-self:flex-end;background:#111827;color:#fff}.links{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.link{display:inline-flex;align-items:center;border:1px solid #dbe3ef;border-radius:999px;padding:5px 8px;text-decoration:none;color:#1d4ed8;background:#f8fbff;font-size:11.5px;font-weight:650}
    ...
    .composer{flex:0 0 auto;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:7px;padding:9px 10px;background:#fff;border-top:1px solid #e5e7eb}.input{width:100%;min-width:0;border:1px solid #cbd5e1;border-radius:10px;padding:9px 10px;color:#111827;background:#fff;outline:none;font-size:16px;line-height:1.25}.input:focus{border-color:#64748b;box-shadow:0 0 0 3px rgba(148,163,184,.18)}.send{min-width:64px;border:0;border-radius:10px;background:#111827;color:#fff;padding:9px 12px;font-weight:700;cursor:pointer;font-size:13px}.send:disabled{opacity:.55;cursor:not-allowed}.note{flex:0 0 auto;padding:0 10px 9px;background:#fff;color:#64748b;font-size:10px;line-height:1.35}
  ...
  const suggestions = document.createElement('div'); suggestions.className = 'suggestions'; ['Arrivano lead da Facebook Ads: li può chiamare?', 'Sono un property manager: mi può servire?', 'Quanto costa?', 'Come funziona il follow-up?'].forEach((question) => { const chip = document.createElement('button'); chip.type = 'button'; chip.className = 'chip'; chip.textContent = question; chip.addEventListener('click', () => submitQuestion(question)); suggestions.appendChild(chip); });
  ...
  addAssistant('Ciao. Posso spiegarti ClerkAI e valutare il tuo caso concreto: settore, chiamate, Facebook Ads, follow-up, WhatsApp, agenda, CRM, chatbot, prezzi e sicurezza. Scrivimi cosa vuoi automatizzare.', [{ label: 'Confronta i piani', href: '/pricing' }]);
